...
simplifyAndConstR :: Pred a -> Pred a
...
simplifyAndNot :: Pred a -> Pred a
simplifyAndNot (PAnd (PNot p) (PNot q)) = PNot (POr p q)
simplifyAndNot p = p

simplifyXorConstL :: Pred a -> Pred a
...
simplifyXorConstR :: Pred a -> Pred a
...
simplifyXorNot :: Pred a -> Pred a
simplifyXorNot (PXor (PNot p) (PNot q)) = PXor q p
simplifyXorNot p = p

simplifyOrConstB :: Pred a -> Pred a
...
simplifyOrConstL :: Pred a -> Pred a
...
simplifyOrConstR :: Pred a -> Pred a
...
simplifyOrNot :: Pred a -> Pred a
simplifyOrNot (POr (PNot p) (PNot q)) = PNot (PAnd p q)
simplifyOrNot p = p

simplifyImplConstB :: Pred a -> Pred a
...
simplifyImplConstL :: Pred a -> Pred a
...
simplifyImplNot :: Pred a -> Pred a
simplifyImplNot (PImpl (PNot p) (PNot q)) = PImpl q p
simplifyImplNot p = p

simplifyImplNot2 :: Pred a -> Pred a
simplifyImplNot2 (PImpl (PNot p) q) = POr p q
simplifyImplNot2 p = p
{-
simplifyAnds :: Pred a -> Pred a
simplifyAnds (PAnds ps) =
  case ps of
    [] -> ptrue
    [p] -> p
    [p, q] -> PAnd p q
...
simplifyMaybeConst2 :: Pred a -> Pred a
...
-- if same PConst on both sides then we know the answer
simplifyEitherConst2 :: Pred a -> Pred a
...
-- if same PConst on both sides then we know the answer
simplifyIxConst2 :: Pred a -> Pred a
...
-- if same PConst on both sides then we know the answer
simplifyHeadConst2 :: Pred a -> Pred a
...
-- if same PConst on both sides then we know the answer
simplifyLastConst2 :: Pred a -> Pred a
...
simplifyForAllConst :: Pred a -> Pred a
...
simplifyExistsConst :: Pred a -> Pred a
...
ss p = foldr (\f p' -> go f p') p
  ...
  where go :: (forall x . Pred x -> Pred x) -> Pred a -> Pred a
        go f p = simplify' f p
-}

-- top down but we want bottom up
simplify' :: forall a . (forall x . Pred x -> Pred x ) -> Pred a -> Pred a
simplify' nat = go
  where
  go :: Pred b -> Pred b
  go px = case nat px of
            ...
            PRegexIP th t rx ra e p -> PRegexIP th t rx ra (go e) (go p)
            PElem as           -> PElem as
